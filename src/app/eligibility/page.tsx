...
Generated by: Loanify AI Decision Engine v2.1`;
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      console.log('Backend unavailable, generating PDF');
    }
    ...
    // Colors
    const primaryBlue = [0, 71, 171];
    const lightBlue = [232, 244, 253];
    const darkGray = [30, 30, 30];
    const lightGray = [201, 209, 217];
    ...
    doc.text('LOANIFY NBFC LIMITED', 105, 20, { align: 'center' });
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text('RBI Licensed NBFC-ND-SI | License No: N-14.03268', 105, 28, { align: 'center' });
    doc.text('AI-Powered Loan Eligibility Assessment Report', 105, 35, { align: 'center' });
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('REPORT INFORMATION', 15, yPos + 5);
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('APPLICANT DETAILS', 15, yPos + 5);
    ...
    const applicantInfo = [
      ...
      ['Monthly Income:', `₹${parseInt(form.income).toLocaleString()}`],
      ['Credit Score:', form.creditScore || 'To be verified']
    ];
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('ELIGIBILITY ASSESSMENT', 15, yPos + 5);
    
    yPos += 20;
    
    // Status Box
    const statusColor = result.status === 'APPROVED' ? [46, 204, 113] : 
                       result.status === 'REJECTED' ? [231, 76, 60] : [243, 156, 18];
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text(`${result.status} - ${result.eligibilityPercentage}% ELIGIBLE`, 105, yPos + 10, { align: 'center' });
    ...
    if (result.status === 'APPROVED') {
      ...
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.text('APPROVED LOAN DETAILS', 15, yPos + 5);
      ...
      const loanDetails = [
        ['Sanctioned Amount:', `₹${result.recommendedAmount?.toLocaleString()}`],
        ['Interest Rate:', `${result.interestRate}% per annum`],
        ['Loan Tenure:', `${form.tenure} months`],
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('RBI COMPLIANCE & ANALYSIS', 15, yPos + 5);
    ...
    const complianceText = [
      '• Assessment based on RBI Guidelines for NBFC lending',
      `• Credit Score: ${form.creditScore >= 700 ? 'Satisfactory' : 'Below standard - requires improvement'}`,
      '• Age Factor: Eligible age group (21-65 years)',
      `• Employment: ${form.employmentType === 'salaried' ? 'Stable salaried employment' : 'Variable income profile'}`,
      '• Loan-to-Income Ratio: Compliant with NBFC norms'
    ];
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('LOANIFY NBFC LIMITED', 105, yPos + 10, { align: 'center' });
    
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('CIN: U65923MH2020PLC123456 | CRISIL Rating: A+ | RBI License: N-14.03268', 105, yPos + 16, { align: 'center' });
    doc.text('Customer Care: 1800-123-LOAN | Email: support@loanify.com', 105, yPos + 22, { align: 'center' });
    
    doc.setFontSize(7);
    doc.text('** CONFIDENTIAL DOCUMENT - FOR APPLICANT USE ONLY **', 105, yPos + 30, { align: 'center' });
    doc.text(`Digital Signature: AI_ENGINE_${Date.now()} | Generated: ${new Date().toLocaleString('en-IN')}`, 105, yPos + 35, { align: 'center' });
    
    // Watermark
    doc.setTextColor(200, 200, 200);
    doc.setFontSize(50);
    doc.text('LOANIFY NBFC', 105, 150, { align: 'center', angle: 45 });
    
    // Save PDF
    doc.save(`Loanify_Professional_Report_${result.referenceId}.pdf`);
  };
